...
namespace ContentVerification
{
	...
	public static class FindFiles
	{
		...
		/// </exception>
		public static IEnumerable<FileData> EnumerateFiles(string path, FileFilter ignored)
		{
			return FindFiles.EnumerateFiles(path, "*", ignored);
		}
		...
		/// </exception>
		public static IEnumerable<FileData> EnumerateFiles(string path, string searchPattern, FileFilter ignored)
		{
			return FindFiles.EnumerateFiles(path, searchPattern, ignored, SearchOption.AllDirectories);
		}
		...
		/// </exception>
		public static IEnumerable<FileData> EnumerateFiles(string path, string searchPattern, FileFilter ignored, SearchOption searchOption)
		{
			if (path == null)
			{
			...
			return new FileEnumerable(fullPath, searchPattern, ignored, searchOption);
		}
		...
		/// <paramref name="filter"/> is a null reference (Nothing in VB)
		/// </exception>
		public static FileData[] GetFiles(string path, string searchPattern, FileFilter ignored, SearchOption searchOption)
		{
			IEnumerable<FileData> e = FindFiles.EnumerateFiles(path, searchPattern, ignored, searchOption);
		...
		private class FileEnumerable : IEnumerable<FileData>
		{
			...
			private readonly FileFilter m_ignored;
			...
			/// operation should include all subdirectories or only the current directory.
			/// </param>
			public FileEnumerable(string path, string filter, FileFilter ignored, SearchOption searchOption)
			{
				m_path = path;
				m_filter = filter;
				m_ignored = ignored;
			...
			{
				return new FileEnumerator(m_path, m_filter, m_ignored, m_searchOption);
			}
		...
		private class FileEnumerator : IEnumerator<FileData>
		{
			...
			private class SearchContext
			{
				...
				public Stack<DirectoryInfo> SubdirectoriesToProcess;

				public SearchContext(int rootlen, DirectoryInfo path)
				{
			...
			/// operation should include all subdirectories or only the current directory.
			/// </param>
			public FileEnumerator(string path, string filter, FileFilter ignored, SearchOption searchOption)
			{
				m_path = new DirectoryInfo(path);
				m_filter = filter;
				m_searchOption = searchOption;
				m_ignored = ignored;
				m_currentContext = new SearchContext(path.EnsureEndsWith("\\").Length, m_path);
			...
			{
				get { return new FileData(m_path.FullName, m_win_find_data); }
			}
			...
			public bool MoveNext()
			{
				...
				if (m_currentContext.SubdirectoriesToProcess == null)
				{
					if (m_hndFindFile == null)
					{
						new FileIOPermission(FileIOPermissionAccess.PathDiscovery, m_path.FullName).Demand();

						string searchPath = Path.Combine(m_path.FullName, m_filter);
				...
				else if (m_searchOption == SearchOption.AllDirectories)
				{
					...
					if (m_currentContext.SubdirectoriesToProcess == null)
					{
						DirectoryInfo[] subDirectories = m_path.GetDirectories();
						m_currentContext.SubdirectoriesToProcess = new Stack<DirectoryInfo>(subDirectories);
					}
					...
					if (m_currentContext.SubdirectoriesToProcess.Count > 0)
					{
						...
						do
						{
							subDir = m_currentContext.SubdirectoriesToProcess.Pop();
							if (!subDir.Attributes.HasFlag(FileAttributes.Hidden))
							{
								string subdirname = subDir.Name.ToLower();
								bool excluded = m_ignored.IsFolderNameIgnored(subdirname);
								if (!excluded)
								{
									string subdirpath = subDir.FullName.Substring(m_currentContext.RootLen).ToLower() + "\\";
									excluded = m_ignored.IsPathIgnored(subdirpath);
									if (!excluded)
										break;
								}
							}
							subDir = null;
						} while (m_currentContext.SubdirectoriesToProcess.Count > 0);
						...
						if (subDir != null)
						{
							...
							m_path = subDir;
							m_hndFindFile = null;
							m_currentContext = new SearchContext(m_currentContext.RootLen, m_path);
							return MoveNext();
						}
					}
