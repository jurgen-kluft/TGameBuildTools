...
int  Skein_256_Test()
{
	...
	const u08b_t message_data1[] = { 0xFF };
	const u08b_t result_data1[] =
	{
	...
	Skein_256_Ctxt_t skein_ctxt1;
	Skein_256_Init(&skein_ctxt1, 256);
	Skein_256_Update(&skein_ctxt1, message_data1, 1);
	Skein_256_Final(&skein_ctxt1, &hash256[0]);
	if (memcmp(result_data1, hash256, sizeof(hash256)) != 0)
		return -1;
...
int  Skein_512_Test()
{
	...
	if (do_test)
	{
		...
		Skein_512_Ctxt_t skein_ctxt1;
		Skein_512_Init(&skein_ctxt1, 512);
		Skein_512_Update(&skein_ctxt1, message_data1, 1);
		Skein_512_Final(&skein_ctxt1, &hash[0]);
		if (memcmp(result_data1, hash, sizeof(hash)) != 0)
			return -1;
	}
	...
	if (do_test)
	{
		...
		Skein_512_Ctxt_t skein_ctxt1;
		Skein_512_Init(&skein_ctxt1, 256);
		Skein_512_Update(&skein_ctxt1, message_data1, sizeof(message_data1));
		Skein_512_Final(&skein_ctxt1, &hash[0]);
		if (memcmp(result_data1, hash, sizeof(hash)) != 0)
			return -1;
	}
...
int Skein_512_Init(Skein_512_Ctxt_t *ctx, size_t hashBitLen)
{
	...
	case  224:
		...
		break;
	case  160:
		memcpy(ctx->X, SKEIN_512_IV_160, sizeof(ctx->X));
		break;
	default:
        /* here if there is no precomputed IV value available */
        /* build/process the config block, type == CONFIG (could be precomputed) */
